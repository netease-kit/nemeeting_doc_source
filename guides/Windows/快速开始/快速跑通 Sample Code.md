本文主要介绍如何快速跑通网易会议组件的 [Windows Sample Code](https://github.com/netease-kit/NEMeeting/tree/main/SampleCode/Windows_macOS)，体验在线的多人视频会议功能。
...
5. 运行工程。
    ...
    $ bash build.bat // build.bat 替成本地Qt环境
    ```
...
`nemeet_sdk` 的路径为 `sdk/bin` 目录下的 `nemeet_sdk` 文件。需要确定本地 `nemeet_sdk` 文件的路径是否正确。
