...
- **会议透传字段**：[`NEStartMeetingParams.extraData`](https://doc.yunxin.163.com/meetingkit/references/web/typedoc/Latest/zh/electron/types/NEStartMeetingParams.html)，用于传递会议级别的自定义数据。
- **成员透传字段**：[`NEMeetingParams.tag`](https://doc.yunxin.163.com/meetingkit/references/web/typedoc/Latest/zh/electron/types/NEMeetingParams.html)，用于传递参会者级别的自定义数据。
...
const params = {
    displayName: '张三',
    extraData: '{"businessId":"10086","projectName":"项目方案讨论","deptCode":"D2023","priority":"high"}',
}
// 使用设置好的参数创建会议
...
const params = {
    displayName: '张三',
    tag: '{"businessId":"10086","projectName":"项目方案讨论","deptCode":"D2023","priority":"high"}',
}
// 使用设置好的参数创建会议
meetingService.startMeeting(params).then(() => {
    // 创建会议成功
...
const meetingInfo = await meetingService.getCurrentMeetingInfo().then((result) => {
    ...
    const extraData = meetingInfo.extraData;
    const tag = meetingInfo.userList[0].tag;
}).catch((error) => {
    // 获取当前会议信息失败
```
