...
// 开启虚拟背景功能。
try {
  await settingsService.enableVirtualBackground(true);
  // 是否支持虚拟背景
  const { data: isVirtualBackgroundEnabled } =
    await settingsService.isVirtualBackgroundEnabled();
  // 获取内置虚拟背景列表
  const { data: builtinVirtualBackgroundList } =
    await settingsService.getBuiltinVirtualBackgroundList();
} catch (error) {
  // 异常处理
}
```
...
try {
  // 设置虚拟背景图片
  await settingsService.setBuiltinVirtualBackgroundList(virtualBackgrounds);
} catch (error) {
  // 异常处理
}
```