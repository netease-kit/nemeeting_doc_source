...
// 初始化外部业务系统
[ExternalBusinessSystem initializeWithAppKey:appKey token:token];
...
```Objective-C
// 登录外部业务系统
[ExternalBusinessSystem loginWithUsername:username password:password completion:^(BOOL success, NSString * _Nullable error) {
    if (success) {
        // 外部业务系统登录成功后，登录网易会议SDK
        [[NE MeetingManager sharedManager] loginWithUserId:userId token:token completion:^(NSInteger code, NSString * _Nullable msg) {
            if (code == 0) {
                // 网易会议SDK登录成功
            } else {
                // 网易会议SDK登录失败
...
```Objective-C
// 登出外部业务系统
[ExternalBusinessSystem logoutWithCompletion:^(BOOL success, NSString * _Nullable error) {
    if (success) {
        // 外部业务系统登出成功后，登出网易会议SDK
        [[NE MeetingManager sharedManager] logoutWithCompletion:^(NSInteger code, NSString * _Nullable msg) {
            if (code == 0) {
                // 网易会议SDK登出成功
            } else {
                // 网易会议SDK登出失败
...
[[NE MeetingManager sharedManager] observeMeetingStatus:^(NEEnumMeetingStatus status) {
    ...
    // 通知外部业务系统当前的会议状态
    switch (status) {
        case NEMeetingStatus::MEETING_STATUS_CONNECTING:
            // 当前正在创建或加入会议
            [ExternalBusinessSystem notifyMeetingConnecting];
            break;
        case NEMeetingStatus::MEETING_STATUS_INMEETING:
            // 当前处于会议中
