...
1. 准备工作。
    ...
    * 若您已集成 IM SDK，需要再同时集成 NEMeetingKit，则需要在指定应用下开通 NERoom 房间组件，操作步骤请参考<a href="https://doc.yunxin.163.com/console/concept/zc3NDYzNzc" target="_blank">开通 NERoom 房间组件</a>。
...
2. <span id="创建一个与IM账号绑定的会议账号">创建一个与 IM 账号绑定的会议账号</span>。
    ...
    假设当前已有一个 IM 账号，其中 imAccid 和 imToken 值分别为 "myAccid" 和 "myToken"。若现在需要复用该 IM 账号，则需要调用 <a href="https://doc.yunxin.163.com/meeting/server-apis/jU0MDAzNTg" target="_blank">创建会议账号</a> 的服务端接口创建一个与该 IM 账号绑定的会议账号，请求时需要在请求参数中将 imAccid 和 imToken 分别设置为该 IM 账号的 imAccid 和 imToken，即 "myAccid" 和 "myToken" 。

    至此您就创建了一个与该 IM 账号绑定的会议账号。同时该服务端接口返回会议账号的 userUuid 和 userToken，可用来登录 NEMeetingKit。

## 复用 IM 账号

自 3.13.0 版本起，NEMeetingKit 会自动检测 IM SDK 是否已登录，登录则直接复用，未登录则不复用。

若您使用 3.13.0 版本之前的会议组件版本，则需要设置 `NEMeetingKitConfig#reuseIM` 字段为 `true` 手动开启 IM 复用功能。具体步骤如下：

1. 初始化 IM SDK 与 NEMeetingKit。
    
    1. 在 `Application` 中初始化 IM SDK。具体请参考 [初始化 IM SDK](https://doc.yunxin.163.com/messaging2/guide/TY4OTgyMDk?platform=client) 文档。
    ...
    2. 初始化 NEMeetingKit。
        ...
        ::: note note
        3.13.0 版本之前的会议组件版本，需要设置 `NEMeetingKitConfig#reuseIM` 字段为 `true` 手动开启 IM 复用功能。3.13.0 之后的版本无此参数，NEMeetingKit 会自动检测 IM SDK 是否已登录，登录则复用，未登录则不复用。
        :::
    ...
    **示例代码如下：**
    ...
    public class MyMeetingApplication extends Application {
        ...
        //1. 初始化 IM SDK，请参考 IM V10 的初始化文档
        ...
        ...
        // 打开 IM 复用开关，3.13.0 版本之前才需要配置，自3.13.0 版本起无此字段，具体请参考说明
        config.reuseIM = true;
        // 其他初始化参数根据实际情况传入
        // ...
        // ...
        NEMeetingKit.getInstance().initialize(this, config, callback);
        }
    }
    ```

2. 登录 IM SDK。具体请参考 [登录 IM SDK](https://doc.yunxin.163.com/messaging2/guide/Dk1MTY4MzA?platform=client) 文档。

    开启复用功能时，应用层需要确保已先使用与该会议账号绑定的 IM 账号登录 IM SDK。登录时机由业务决定，只需要确保在进行 NEMeetingKit 登录前完成 IM 登录即可。

    请通过 IM 账号的 imAccid 和 imToken 完成登录，**示例代码如下：**

    ```
    NIMClient.getService(V2NIMLoginService.class).login(imAccid, imToken);
    ```

3. 登录 NEMeetingKit。

    使用对应的会议账号登录 NEMeetingKit，请通过<a href="#创建一个与 IM 账号绑定的会议账号">创建会议账号</a>时返回的 userUuid（对应 accountId） 和 userToken（对应 accountToken） 完成登录，**示例代码如下：**

    ```
    //以使用账号 ID 和 Token 登录为例
    NEMeetingKit.getInstance().login(accountId, accountToken, callback);
    ```
