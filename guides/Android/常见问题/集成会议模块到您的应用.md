...
// 初始化外部业务系统
ExternalBusinessSystem.init(config.appKey, token);
```
...
```Java
// 登录外部业务系统
ExternalBusinessSystem.login(username, password, new Callback() {
    @Override
    public void onSuccess() {
        // 外部业务系统登录成功后，登录网易会议SDK
        NEMeetingKit.getInstance().login(userId, token, new NECallback<Void>() {
            @Override
            public void onResult(int resultCode, String resultMsg, Void result) {
                if (resultCode == NEMeetingError.ERROR_CODE_SUCCESS) {
                    // 网易会议SDK登录成功
...
```Java
// 登出外部业务系统
ExternalBusinessSystem.logout(new Callback() {
    @Override
    public void onSuccess() {
