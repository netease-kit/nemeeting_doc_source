...
当用户调用 NEMeetingKit <a href="https://doc.yunxin.163.com/meeting/guide/DgxNTg1Njg?platform=h5" target="_blank">登录接口</a> 传入会议账号进行登录时，NEMeetingKit 内部会查询并获取到该会议账号唯一绑定的 IM 账号，并自动登录 IM SDK 以建立起长连接通道。此时，如果上层业务同时也集成了 IM SDK，并已经进行了 IM SDK 登录操作，那么由于 IM SDK 同时只允许登入一个账号，因此会造成此次 NEMeetingKit 登录失败。
